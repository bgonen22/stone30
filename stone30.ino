...
void loop()
{
  if (tlc_fadeBufferSize < TRACE_SIZE /*TLC_FADE_BUFFER_LENGTH - 2*/) {
    if (!tlc_isFading(channel)) {
      ...
      tlc_addFade(channel, 0.5*maxValue, maxValue, endMillis, endMillis +TIME_ON_TOP);
      tlc_addFade(channel, maxValue, 0.5*maxValue, endMillis + TIME_ON_TOP, endMillis + 2*TIME_ON_TOP);
      tlc_addFade(channel, 0.5*maxValue, 0, endMillis + 2*TIME_ON_TOP, endMillis + 2*TIME_ON_TOP + duration);
    }
